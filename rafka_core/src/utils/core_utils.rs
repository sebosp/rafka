...
use crate::log::log_manager::LogError;
...
pub fn replace_suffix(s: &str, current_suffix: &str, new_suffix: &str) -> Result<String, LogError> {
    match s.find(current_suffix) {
        Some(idx) => {
            let (without_suffix, _) = s.split_at(idx);
            Ok(format!("{}{}", without_suffix, new_suffix))
        },
        None => Err(LogError::UnexpectedSuffix(current_suffix.to_string(), s.to_string())),
    }
}
